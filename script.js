/* ============================================
   ...
   script.js — Firebase + Tienda completa
   ============================================ */

import { initializeApp } from "https://www.gstatic.com/firebasejs/10.12.0/firebase-app.js";
import { getFirestore, collection, getDocs, addDoc, setDoc, deleteDoc, doc } from "https://www.gstatic.com/firebasejs/10.12.0/firebase-firestore.js";
...
const defaultProducts = [
  ...
  { id: 'p3', name: 'Macarons Franceses x10', category: 'reposteria', price: 45000, oldPrice: 0, stock: 25, img: 'https://images.unsplash.com/photo-1569864358642-9d1684040f43?w=400&q=80', desc: 'Macarons elaborados con la receta francesa tradicional. Rellenos de ganache y buttercream. Colores y sabores variados.', featured: false },
  { id: 'p4', name: 'Caja de Regalo Romántica', category: 'detalles', price: 65000, oldPrice: 80000, stock: 15, img: 'https://images.unsplash.com/photo-1513201099705-a9746072231e?w=400&q=80', desc: 'Caja de regalo decorada con lazo y detalles especiales. Incluye tarjeta personalizada. Perfecta para San Valentín, aniversarios y cumpleaños.', featured: true },
  { id: 'p5', name: 'Bouquet de Chocolates', category: 'detalles', price: 75000, oldPrice: 0, stock: 12, img: 'https://images.unsplash.com/photo-1549007994-cb92caebd54b?w=400&q=80', desc: 'Ramo de chocolates premium decorado artesanalmente. Incluye variedad de chocolates belgas y nacionales. Ideal para regalar.', featured: false },
  { id: 'p6', name: 'Collar de Perlas Artesanal', category: 'joyeria', price: 95000, oldPrice: 130000, stock: 10, img: 'https://images.unsplash.com/photo-1599643478518-a784e5dc4c8f?w=400&q=80', desc: 'Collar artesanal elaborado con perlas cultivadas y cierre dorado. Diseño elegante y atemporal. Incluye caja de regalo.', featured: true },
  { id: 'p7', name: 'Aretes de Resina Floral', category: 'joyeria', price: 35000, oldPrice: 45000, stock: 20, img: 'https://images.unsplash.com/photo-1535632066927-ab7c9ab60908?w=400&q=80', desc: 'Aretes artesanales de resina con flores naturales preservadas. Piezas únicas e irrepetibles. Livianos y cómodos.', featured: false },
  { id: 'p8', name: 'Centro de Mesa Floral', category: 'decoraciones', price: 120000, oldPrice: 0, stock: 8, img: 'https://images.unsplash.com/photo-1487530811015-780a9d47e8be?w=400&q=80', desc: 'Centro de mesa elaborado con flores artificiales de alta calidad. Ideal para bodas, quinceañeras y eventos especiales. Personalizable en colores.', featured: true },
  { id: 'p9', name: 'Globos Decorativos Set x20', category: 'decoraciones', price: 28000, oldPrice: 35000, stock: 50, img: 'https://images.unsplash.com/photo-1527529482837-4698179dc6ce?w=400&q=80', desc: 'Set de globos metalizados y de látex para decoración. Colores variados. Incluye cintas decorativas. Para toda ocasión.', featured: false },
  { id: 'p10', name: 'Velas Artesanales Aromáticas', category: 'otros', price: 42000, oldPrice: 55000, stock: 35, img: 'https://images.unsplash.com/photo-1602607242655-a0f0cc99dc44?w=400&q=80', desc: 'Velas elaboradas con cera de soja y esencias naturales. Aromas: lavanda, vainilla, rosa y jazmín. Tiempo de quemado: 40 horas.', featured: false }
];
...
function showLoading(show) {
  ...
  if (!loader && show) {
    ...
    loader.innerHTML = `<div class="loader-spinner"></div><p class="loader-text">Cargando productos con amor... 🎂</p>`;
...
window.goHome = function() { window.setCategory('all'); window.scrollTo({top:0,behavior:'smooth'}); };
window.scrollToCatalog = function() { document.getElementById('catalog')?.scrollIntoView({behavior:'smooth'}); };
...
window.addToCartFromModal = function() { if (!currentProductId) return; addToCart(currentProductId, currentQty); closeProductModal(); };
...
function addToCart(id, qty = 1) {
  ...
  if (existing) {
    ...
    existing.qty = newQty;
  } else { cart.push({ id, qty, price: p.price, name: p.name, img: p.img }); }
  saveCart(); updateCartBadge();
  showToast(`🎂 ${p.name} agregado al carrito`, 'success');
}
window.addToCart = addToCart;

window.removeFromCart = function(id) { cart = cart.filter(x => x.id !== id); saveCart(); updateCartBadge(); renderCart(); };
...
function renderCheckoutSummary() {
  ...
  el.innerHTML = `
    ...
    ${cart.map(i => { const p = products.find(x => x.id === i.id); const price = p ? p.price : i.price;
      return `<div class="cs-item"><span>${i.name} x${i.qty}</span><span>${formatPrice(price * i.qty)}</span></div>`; }).join('')}
    <div class="cs-total"><span>💰 SUBTOTAL</span><span>${formatPrice(total)}</span></div>
    <p style="font-size:.78rem;color:var(--primary);margin-top:8px;font-style:italic;">🚚 El costo de envío será acordado por WhatsApp según tu ubicación.</p>`;
}
...
window.submitOrder = async function(e) {
  ...
  const total = cart.reduce((s, i) => { const p = products.find(x => x.id === i.id); return s + (p ? p.price : i.price) * i.qty; }, 0);
  const items = cart.map(i => { const p = products.find(x => x.id === i.id); const price = p ? p.price : i.price; return `• ${i.name} x${i.qty} = ${formatPrice(price * i.qty)}`; }).join('\n');
  ...
  try {
    await addDoc(collection(db, 'orders'), { name, address, city, phone, email, notes, items: [...cart], total, date: new Date().toISOString() });
  } catch (err) {
    const orders = JSON.parse(localStorage.getItem('mtdy_orders') || '[]');
    orders.unshift({ name, address, city, phone, email, notes, items: [...cart], total, date: new Date().toISOString() });
...
window.switchUserTab = function switchUserTab(tab) {
...
window.doLogin = function() {
  ...
  currentUser = found; localStorage.setItem('mtdy_user', JSON.stringify(found)); updateUserUI(); window.closeUserModal(); showToast(`Bienvenida, ${found.name}! 💕`, 'success');
};
window.doRegister = function() {
  ...
  const users = JSON.parse(localStorage.getItem('mtdy_users') || '[]');
  if (users.find(u => u.username === uname || u.email === email)) { showToast('Usuario o correo ya registrado', 'error'); return; }
  ...
  currentUser = newUser; localStorage.setItem('mtdy_user', JSON.stringify(newUser)); updateUserUI(); window.closeUserModal(); showToast(`Bienvenida, ${name}! 🎂`, 'success');
};
window.doLogout = function() { currentUser = null; localStorage.removeItem('mtdy_user'); updateUserUI(); window.closeUserModal(); showToast('Sesión cerrada', 'info'); };
...
function loadSocialLinks() {
  ...
  const wa = socials.whatsapp || '573165389152';
...
document.addEventListener('keydown', e => { if (e.key === 'Escape') ['product-modal','cart-modal','checkout-modal','user-modal','product-form-modal'].forEach(closeModal); });
